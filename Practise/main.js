...
const dataURL = "http://api.exchangeratesapi.io/v1";

const initiallyDisplayedCurrencies = ["USD", "EUR", "GBP", "JPY", "UAH"];
let baseCurrency;
...
function setNewBaseCurrency(newBaseCurrencyLI){
  ...
  currenciesList.querySelectorAll(".currency").forEach(currencyLI =>{
    const currencyRate = currencies.find(currency => currency.abbreviation===currencyLI.id).rate;
    const exchangeRate = currencyLI.id===baseCurrency ? 1 : (currencyRate/baseCurrencyRate ).toFixed(4);
    currencyLI.querySelectorAll(".base-currency-rate").textContent = `1 ${baseCurrency} = ${exchangeRate} ${currencyLI.id}`
  });
}
...
function params(paramsObj){
  return new URLSearchParams({
    access_key: 'd5a905a6bb625c91065b148e160ed3e4',
    ... paramsObj
  });
}
function getLatest(options){
  fetch(`${dataURL}/latest?${params(options)}`)
    .then(res => res.json())
    .then(data => {
